...
func (s *CourseService) CreateCourse(input models.CreateCourseDTO) (*models.Course, error) {
	...
	if err := s.validate.Struct(input); err != nil {
		return nil, utils.NewValidationError(utils.FormatValidationErrors(err.(validator.ValidationErrors)))
	}
...
func (s *CourseService) FilterCourses(dept string, level int, semester int) ([]models.Course, error) {
	var courses []models.Course
	
	query := s.db.
		Where("level_id = ? AND semester = ?", level, semester).
